...
configure_php() {
    ...
    # Chercher d'abord le PHP générique, puis les versions spécifiques
    if [ -f "/etc/php/cli/php.ini" ]; then
        php_ini="/etc/php/cli/php.ini"
        fpm_ini="/etc/php/fpm/php.ini"
        debug "Utilisation de PHP générique"
    elif [ -f "/etc/php/8.4/cli/php.ini" ]; then
    ...
    if [ -f "/etc/systemd/system/multi-user.target.wants/php8.4-fpm.service" ]; then
        systemctl restart php8.4-fpm 2>/dev/null || true
        debug "PHP 8.4 FPM redémarré"
    elif [ -f "/etc/systemd/system/multi-user.target.wants/php8.2-fpm.service" ]; then
